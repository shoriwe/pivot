...
func (logger *Logger) LogRegistration(context *gin.Context, user *objects.User, succeed bool) {
	if succeed {
		logger.debugLogger.Printf("%s successful registered as %s", context.Request.RemoteAddr, user.Email)
	} else {
		logger.debugLogger.Printf("%s failed to register as %s", context.Request.RemoteAddr, user.Email)
	}
}
